...
def elevator(start: tuple[int, ...]) -> int:
    ...
    initial_steps = 0
    initial_floor = 1
    initial_state = normalize(start)
    
    queue = [(initial_steps, initial_floor, initial_state)]
    visited = {(initial_floor, initial_state)}
    ...
    while queue:
        ...
        for num_items_to_move, floor_direction in product((1, 2), (-1, 1)):
            for selected_items in combinations(items_on_floor, num_items_to_move):
                ...
                new_state = tuple(
                    item_floor + floor_direction if item_idx in selected_items else item_floor
                    for item_idx, item_floor in enumerate(current_state)
                )
                ...
                if (next_floor, normalized_new_state) not in visited:
                    heappush(queue, (current_steps + 1, next_floor, normalized_new_state))
