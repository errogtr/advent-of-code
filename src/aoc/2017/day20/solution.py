...
def main(input_path):
    ...
    for (i1, p1), (i2, p2) in combinations(enumerate(particles), 2):
        ...
        B_x, B_y, B_z = (v_x2 - v_x1 + (a_x2 - a_x1) / 2), (v_y2 - v_y1 + (a_y2 - a_y1) / 2), (v_z2 - v_z1 + (a_z2 - a_z1) / 2)
        ...
        if t_x == t_y == t_z and t_x > 0: # or (t_x == inf and t_y == t_z) or (t_y == inf and t_x == t_z) or (t_z == inf and t_x == t_y) or (t_x == t_y == inf) or (t_x == t_z == inf) or (t_y == t_z == inf):
            print(t_x)
